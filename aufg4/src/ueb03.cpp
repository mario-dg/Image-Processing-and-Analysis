...
cv::Matx33f ueb03::calcUprightMatrix(std::vector<int> values, cv::Mat img)
{
    ...
    int x0 = std::floor(values.at(2) - img.cols / 2.0f + 0.5f);
    int y0 = std::floor(values.at(3) - img.rows / 2.0f + 0.5f);
    int x1 = std::floor(values.at(4) - img.cols / 2.0f + 0.5f);
    int y1 = std::floor(values.at(5) - img.rows / 2.0f + 0.5f);
    int x2 = std::floor(values.at(6) - img.cols / 2.0f + 0.5f);
    int y2 = std::floor(values.at(7) - img.rows / 2.0f + 0.5f);
    //Zielkoordinaten
    int xd0 = std::floor(-values.at(0) / 2.0f + 0.5f);
    int yd0 = std::floor(values.at(1) / 2.0f + 0.5f);
    int xd1 = std::floor(-values.at(0) / 2.0f + 0.5f);
    int yd1 = std::floor(-values.at(1) / 2.0f + 0.5f);
    int xd2 = std::floor(values.at(0) / 2.0f + 0.5f);
    int yd2 = std::floor(-values.at(1) / 2.0f + 0.5f);
...
std::vector<int> ueb03::calcNewImageSize(cv::Mat img, cv::Matx33f transformMat)
{
    ...
    cv::Mat topRight = (cv::Mat_<float>(3, 1, CV_32F) << img.cols, 0, 1);
    cv::Mat bottomRight = (cv::Mat_<float>(3, 1, CV_32F) << img.cols, img.rows, 1);
    cv::Mat bottomLeft = (cv::Mat_<float>(3, 1, CV_32F) << 0, img.rows, 1);
    ...
    int width = std::round(maxX - minX);
    int height = std::round(maxY - minY);
...
cv::Mat ueb03::transform(cv::Matx33f transMat, std::vector<int> size, cv::Mat img, argp::INTERPOLATION interpol, argp::EDGE edgeOpt, float edgeValue)
{
    ...
    //Zielbild
    cv::Mat toReturn(size.at(1), size.at(0), CV_8UC3);
    edgeValue = CLAMP(edgeValue, 0.0f, 1.0f);

    cv::Vec3b edgeValueVec = {(unsigned char) (edgeValue * 255), (unsigned char) (edgeValue * 255), (unsigned char) (edgeValue * 255)};

    util::for_each_pixel(toReturn, [invTransMat, img, edgeOpt, edgeValueVec, size, interpol](cv::Mat &toReturn, int y, int x) -> void {
        //Verschiebung in den Ursprung (Index != Position)
        cv::Mat temp = (cv::Mat_<float>(1, 3) << x + 0.5f - size.at(0) / 2.0f, y + 0.5f - size.at(1) / 2.0f, 1);
        //Rueckverschiebung
        float newY = (invTransMat * temp)[1] - 0.5f + img.rows / 2.0f;
        float newX = (invTransMat * temp)[0] - 0.5f + img.cols / 2.0f;
        ...
        if (interpol == argp::INTERPOLATION_NEAREST)
        {
            ...
            if (newX < 0.0f || newX >= img.cols || newY < 0.0f || newY >= img.rows)
            {
                ...
                case argp::EDGE_VALUE:
                    toReturn.at<cv::Vec3b>(y, x) = edgeValueVec;
                    break;
                case argp::EDGE_REPLICATE:
                    toReturn.at<cv::Vec3b>(y, x) = img.at<cv::Vec3b>(ueb02::clampToEdge(newY, 0, img.rows - 1), ueb02::clampToEdge(newX, 0, img.cols - 1));
                    break;
                case argp::EDGE_REFLECT_11:
                    toReturn.at<cv::Vec3b>(y, x) = ueb02::reflect(argp::EDGE_REFLECT_11, img, cv::Mat::ones(1, 1, CV_32FC1), 0, 0, newY, newX);
                    break;
                case argp::EDGE_REFLECT_101:
                    toReturn.at<cv::Vec3b>(y, x) = ueb02::reflect(argp::EDGE_REFLECT_101, img, cv::Mat::ones(1, 1, CV_32FC1), 0, 0, newY, newX);
                    break;
                }
            }
            else
            {
                toReturn.at<cv::Vec3b>(y, x) = img.at<cv::Vec3b>(newY, newX);
            }
        }
        ...
        else if (interpol == argp::INTERPOLATION_BILINEAR)
        {
            ...
            for (unsigned int i = 0; i < neighbours.size() / 2; i++)
            {
                if (neighbours.at(2 * i) < 0.0f || neighbours.at(2 * i) >= img.cols || neighbours.at(2 * i + 1) < 0.0f || neighbours.at(2 * i + 1) >= img.rows)
                {
                    ...
                    case argp::EDGE_NULL:
                    case argp::EDGE_VALUE:
                        values.push_back(edgeValueVec);
                        break;
                    case argp::EDGE_REPLICATE:
                        values.push_back(img.at<cv::Vec3b>(ueb02::clampToEdge(neighbours.at(2 * i + 1), 0, img.rows - 1), ueb02::clampToEdge(neighbours.at(2 * i), 0, img.cols - 1)));
                        break;
                    case argp::EDGE_REFLECT_11:
                        values.push_back(ueb02::reflect(argp::EDGE_REFLECT_11, img, cv::Mat::ones(1, 1, CV_8UC3), 0, 0, neighbours.at(2 * i + 1), neighbours.at(2 * i)));
                        break;
                    case argp::EDGE_REFLECT_101:
                        values.push_back(ueb02::reflect(argp::EDGE_REFLECT_101, img, cv::Mat::ones(1, 1, CV_8UC3), 0, 0, neighbours.at(2 * i + 1), neighbours.at(2 * i)));
                        break;
                    }
                }
                else
                {
                    values.push_back(img.at<cv::Vec3b>(neighbours.at(2 * i + 1), neighbours.at(2 * i)));
                }
            }

            //Gewichtung der Intensitäten
            toReturn.at<cv::Vec3b>(y, x) = deltaX * deltaY * values.at(3) + (1 - deltaX) * deltaY * values.at(2) + deltaX * (1 - deltaY) * values.at(1) + (1 - deltaX) * (1 - deltaY) * values.at(0);
        }
    });
