...
float ueb02::reflect(argp::EDGE edgeopt, cv::Mat img, cv::Mat filter, int i, int k, int y, int x)
{
    ...
    return filter.at<float>(i, k) * img.at<float>(newY, newX);
}
            ...
            for (k = 0; k < filter.cols; k++)
            {
                if (((y - i + filter.rows / 2) < 0) || ((y - i + filter.rows / 2) >= img.rows) || ((x - k + filter.cols / 2) < 0) || ((x - k + filter.cols / 2) >= img.cols))
                {
                    ...
                    case argp::EDGE_REPLICATE:
                        result.at<float>(y, x) += img.at<float>(ueb02::clampToEdge(y - i + filter.rows / 2, 0, img.rows - 1), ueb02::clampToEdge(x - k + filter.cols / 2, 0, img.cols - 1)) * filter.at<float>(i, k);
                        break;
                    case argp::EDGE_REFLECT_11:
                        result.at<float>(y, x) += ueb02::reflect(argp::EDGE_REFLECT_11, img, filter, i, k, y, x);
                        break;
                    case argp::EDGE_REFLECT_101:
                        result.at<float>(y, x) += ueb02::reflect(argp::EDGE_REFLECT_101, img, filter, i, k, y, x);
                        break;
                    }
                }
                else
